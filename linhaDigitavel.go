...
func main() {
	...
	** cod_empresa_banco (convenio) = 7 digitos
	** nossoNumero = 12 digitos
	** carteira = 4 digitos
	** vencimento = Padrao JSON
	** valor_boleto = Padrao JSON
	*
	 */
	fmt.Println("Santander:", gerarLinhaDigitavelSantander("033", "617756-5",
		"123456789098", "1234", "2002-12-01T12:18:07.990Z", "835,89"))

	/*
	** Parametros: Gerar Linha Digitavel Caixa Economica Federal
	*
	** codigo_banco = 3 Digitos (104)
	** cod_empresa_banco (convenio) = 8 digitos
	** nossoNumero = 10 digitos
	** agencia = 4 digitos
	** operacao = 3 digitos
	** vencimento = Padrao JSON
	** valor_boleto = Padrão JSON
	*
	 */
	fmt.Println("Caixa Economica Federal:", gerarLinhaDigitavelCaixaEconomica("104", "12345678",
	...
	** cod_empresa_banco (conta) = 7 digitos
	** nossoNumero = 11 digitos
	** agencia = 4 digitos
	** carteira = 2 digitos
	** vencimento = Padrao JSON
	** valor_boleto = Padrão JSON
	...
	** cod_empresa_banco (conta) = 8 digitos
	** convenio = 6 digitos ou 7 digitos
	** nossoNumero = 5 digitos ou 10 digitos
	** agencia = 4 digitos
	** carteira = 2 digitos
	** vencimento = Padrao JSON
	** valor_boleto = Padrão JSON
	** Para convenio de 6 digitos o nosso numero = 5
	** Para convenio de 7 digitos o nosso numero = 10
	*
	 */
	fmt.Println("Banco do Brasil:", gerarLinhaDigitavelBancoDoBrasil("001", "12345678", "1234567", "1234567890", "1234", "12", "2002-12-01T12:18:07.990Z", "835,89"))
}
...
func gerarLinhaDigitavelCaixaEconomica(codigo_banco string, cod_empresa_banco string,
...
func gerarLinhaDigitavelBradesco(codigo_banco string, cod_empresa_banco string,
	...
	conta := formatarConvenio(cod_empresa_banco)
	campoLivreBradesco := gerarCampoLivreBradesco(agencia, carteira, nossoNumero, conta)
...
func gerarLinhaDigitavelBancoDoBrasil(codigo_banco string, cod_empresa_banco string, convenio string,
	nossoNumero string, agencia string, carteira string, vencimento string, valor_boleto string) string {
	codigo := formatarConvenio(cod_empresa_banco)

	campoLivreBancoDoBrasil := ""
	if len(convenio) > 6 {
		campoLivreBancoDoBrasil = gerarCampoLivreBancoBrasil7Digitos(convenio, nossoNumero, carteira)
	} else {
		campoLivreBancoDoBrasil = gerarCampoLivreBancoBrasil6Digitos(convenio, nossoNumero, agencia, codigo, carteira)
	}
